...
public class RedBlackTree<T extends Comparable> {
     ...
     *
     *      如果是从情况1开始发生的，必然会走完情况2和3，也就是说这是一整个流程，当然咯，实际中可能不一定会从情况1发生，
     *      如果从情况2开始发生，那再走个情况3即可完成调整，如果直接只要调整情况3，那么前两种情况均不需要调整了。
        ...
        while (x != null && x != root && x.parent.color == RED) {
            ...
            if (parentOf(x).isLeftChild()) {
                ...
                if (colorOf(uncle) == RED) {
                    ...
                    if (x.isRightChild()) {
                        x = parentOf(x);
                        // x和x的父节点不在一条“直线路径”上，对x的父节点左旋一下弄成看起来是一条直线的路径
                        rotateLeft(x);
                    }
                    setColor(parentOf(x), BLACK);
                    setColor(grandfatherOf(x), RED);
    ...
    private void rotateLeft(Node p) {
        if (p != null) {
            ...
            //2. 将p的父节点(非空时)赋给r的父节点，同时更新p的子节点为(左或右)设置为r
            r.parent = p.parent;
            if (p.parent == null) {
                root = r;//如果x的父节点为空(即x为根节点)，则将y设为根节点
            } else if (p == p.parent.left) {//如果p是左子节点
                p.parent.left = r;//则也将r设为左子节点
            } else {
                p.parent.right = r;//否则将r设为右子节点
            }
            //3. 将r的左子节点设为p，将p的父节点设为r的父节点
            r.left = p;
            p.parent = r;
        }
    }
    ...
    private void rotateRight(Node p) {
        //1. 将l的右子节点赋给p的左子节点，并将p赋给L右子节点的父节点(y左子节点非空时)
        ...
        if (p != null) {
            ...
            //2. 将p的父节点(非空时)赋给L的父节点，同时更新p的子节点为(左或右)
